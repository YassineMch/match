...
if __name__=='__main__':
    rospy.init_node("amcl_transformed")
    
    tfBuffer = tf2_ros.Buffer(rospy.Duration(100.0))
    tf_listener = tf2_ros.TransformListener(tfBuffer)
    
    sub_pose = rospy.Subscriber("/amcl_pose", PoseWithCovarianceStamped, callback_pose)
    pub = rospy.Publisher("/transformed_AMCL_pos", PoseWithCovarianceStamped, queue_size=10)
    rospy.spin()